...
try:
    ...
    Self: TypeVar = TypeVar("Self", bound="NamespaceKey")
...
class NamespaceKey(str):
    ...
    def __eq__(self, value: object) -> bool:
        if isinstance(value, NamespaceKey):
    ...
    @property
    def id(self) -> str:
        """The full ``namespace:name`` key as string.

        :return: the string ``namespace:name`` key (including "minecraft" prefix)
        :rtype: str
        """
    ...
    @property
    def type(self) -> str:
        """Similar to the :attr:`.id` but excluding the "minecraft" namespace but including non-vanilla namespaces. This is always valid id as the "minecraft" namespace is assumed by default if no namespace is provided.

        :return: the key :attr:`.name` of ``namespace:name`` if key is from vanilla namespace or the :attr:`.id` if non-vanilla key
        :rtype: str
        """
    ...
    def name(self) -> str:
        ...
        .. caution::
           ...
           The returned string might not be a valid id without its namespace if the namespace is something other than "minecraft"!
           Use :attr:`.type` instead, which will remove the "minecraft" prefix but not other namespaces.

        :return: the string type of ``namespace:name`` of given key without the namespace
        :rtype: str
        """
        t = self.type
        if ":" in t:
    ...
    def namespace(self) -> str:
        """The namespace string of ``namespace:name``. This returns "minecraft" for vanilla keys.

        :return: the string namespace of ``namespace:name`` of given key
        :rtype: str
        """
        if ":" in self:
            return self[: self.index(":")]
    ...
    def datastr(self) -> str:
        """The data/components string of given key in form '[component1=value1,component2=value2]'.
        Looks like ``'[]'`` if this key has no component data.

        :return: the string data/components of ``namespace:name[component1=value1,...]`` of given key
        :rtype: str
        """
        if "[" in self:
            return self[self.index("[") :]
    ...
    def hasData(self) -> bool:
        """Check if the key has component data, but does not actually parse the data. For that use :func:`getData`."""
    ...
    def equals(self, value: object) -> bool:
        ...
        :param value: the string or key to compare against
        :type value: object
        :return: whether the supplied key has the same id and component data
        :rtype: bool
        """
        if isinstance(value, str) and not isinstance(value, NamespaceKey):
    ...
    def withId(self, type_id: str | Self) -> Self:
        dstr = self.datastr if self.hasData else ""
        if isinstance(type_id, self.__class__):
            return self.__class__(type_id.type + dstr)
        elif isinstance(type_id, str):
            return self.__class__(self.__class__(type_id).type + dstr)
        else:
            raise ValueError(
                f"Unknown id type {type(type_id).__name__}, expected str or {self.__class__.__name__}"
            )

    def withData(
        self,
        data: dict[str, bool | int | str] | str | ComponentData | NbtCompound | Self | None = None,
    ) -> Self:
        if data is None:
            return self.__class__(self.type)
        elif isinstance(data, self.__class__):
            return self.__class__(self.type + (data.datastr if data.hasData else ""))
        elif isinstance(data, ComponentData):
            return self.__class__(self.type + (str(data) if data else ""))
        elif isinstance(data, NbtCompound):
            return self.__class__(self.type + (str(data.asComponentData()) if data else ""))
        elif isinstance(data, str):
            d = ComponentData.parse(data)
            return self.__class__(self.type + (str(d) if d else ""))
        elif isinstance(data, Mapping):
            d = ComponentData(data)
            return self.__class__(self.type + (str(d) if d else ""))
        else:
            raise ValueError(
                f"Unknown data type {type(data).__name__}, expected dict, str, {self.__class__.__name__}, ComponentData or None"
            )

    def withMergeData(
        self, data: dict[str, bool | int | str] | str | ComponentData | Self
    ) -> Self:
        if isinstance(data, self.__class__):
            ...
            raise ValueError(
                f"Unknown data type {type(data).__name__}, expected dict, str, {self.__class__.__name__} or ComponentData"
            )

        return self.withData(self.getData() | d)


class Block(NamespaceKey):
    ...
    def withMergeData(
        ...
        :rtype: Block
        """
        return super().withMergeData(data)


class EntityType(NamespaceKey):
    """works like :class:`Block`, subtype of Python :class:`str`"""
